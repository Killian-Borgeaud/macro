...
Second-brain workspace for HSG Macroeconomics III (Spring 2026, Prof. Cozzi). Generates HTML lectures, MCQ quizzes, computational exercises, exam simulations, and essay-grading round-trips from extracted course PDFs.
...
2. Identify the task type:
   ...
   - **Skill invocation** (`/lecture`, `/quiz`, `/exercise`, `/exam`, `/essay-q`): the skill's SKILL.md handles loading via `@import`s.
...
*Why: the template + JSON pattern keeps token use low (no HTML boilerplate regeneration), keeps output consistent across runs, and lets templates evolve without touching past outputs.* A Step 6 hook will block direct writes to `outputs/**/*.html` mechanically; until then, skills enforce this themselves via `docs/OUTPUT_CONVENTIONS.md`.
...
│   └── HOOKS.md                 ← Step 6 hook design sketch (reference only)
├── tools/
│   ├── setup/                   ← one-time (extract_all.py, slice_textbook.py)
│   ├── build_<type>.py          ← build scripts: lecture, quiz, exercise, exam, essay
│   └── templates/               ← HTML templates merged with JSON payloads
├── outputs/                     ← generated material (only via tools/build_*.py)
│   └── {lectures, quizzes, exercises, exams, essays}/
└── .claude/
    ├── settings.json            ← permissions, hooks (Step 6)
    ├── settings.local.json
    ├── skills/                  ← /lecture, /quiz, /exercise, /exam, /essay-q
    └── hooks/                   ← Step 6 deliverables
```
</folder_map>

<skills>
Five skills live in `.claude/skills/`. They are written in Step 3, after `docs/EXAM_PROFILE.md` exists (Step 4).

- **`/lecture`** — HTML study document for a topic or module
- **`/quiz`** — conceptual MCQs (4 choices, calibrated style distribution)
- **`/exercise`** — single-block compute/derive problems with hints and worked solutions
- **`/exam`** — mixed-format exam simulator with scenario chains (tuned to `docs/EXAM_PROFILE.md`)
- **`/essay-q`** — round-trip: skill writes `prompt.md`, user writes `answer.md`, skill produces `grading.html`
</skills>
...
- **Hook design (planned, not yet implemented):** `docs/HOOKS.md`
</navigation>
